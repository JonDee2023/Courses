document.addEventListener("DOMContentLoaded", function (){
    let suppliedPassword = document.getElementById("new-password")
...
document.addEventListener("DOMContentLoaded", function (){
    let suppliedPassword = document.getElementById("confirm-password")
...
async function saveNewPassword(e) {
    ...
    try {
        const response = await fetch("http://localhost:4000/api/savenewpassword", {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
            alert(data.message);
            window.location.href = "wrktr-login-full.html";
        } else {
            alert(data.message);
        }
